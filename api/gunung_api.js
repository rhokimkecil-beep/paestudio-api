...
const JSON_CHUNK_SIZE = 500000; // 500KB per chunk
...
async function initDb(db) {
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS route_json_chunks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_rjc ON route_json_chunks(route_id, chunk_index)`);
  ...
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_chunks ON chunks(upload_id, chunk_index)`);
}
...
function sendErr(res, msg, code) {
  return res.status(code || 400).json({ success: false, error: msg });
}

function checkToken(req, res) {
  const token = req.query.token || "";
  if (token !== API_TOKEN) {
    sendErr(res, "Token tidak valid", 403);
    return false;
  }
  return true;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return sendErr(res, "DB init error: " + e.message, 500);
  }
    ...
    switch (action) {
      ...
      case "list_routes": {
        ...
        const routes = await db.execute({
          sql: `SELECT id, nama, deskripsi, frame_count, file_size, views, created_at FROM routes ORDER BY id DESC LIMIT ? OFFSET ?`,
          args: [limit, offset],
        });
      ...
      case "search": {
        ...
        if (!q) return sendErr(res, '"q" wajib diisi');
      ...
      case "get_route_info": {
        ...
        if (!id) return sendErr(res, '"id" wajib diisi');
        const route = await db.execute({
          sql: `SELECT * FROM routes WHERE id = ?`,
          args: [id],
        });
        if (!route.rows.length) return sendErr(res, "Route tidak ditemukan", 404);
        return sendOk(res, { route: route.rows[0] });
      }
      ...
      case "get_json": {
        ...
        if (!json) return sendErr(res, "JSON tidak ditemukan", 404);
      ...
      case "get_json_preview": {
        ...
        if (!id) return sendErr(res, '"id" wajib diisi');
        const json = await getJsonChunked(db, id);
        if (!json) return sendErr(res, "Route tidak ditemukan", 404);
        const frames = JSON.parse(json);
        const preview = Array.isArray(frames) ? frames.slice(0, limit) : [];
        return sendOk(res, { frames: preview, shown: preview.length });
      }
      ...
      case "upload_json_raw": {
        ...
        if (!nama) return sendErr(res, '"nama" wajib diisi');

        let body = typeof req.body === "string" ? req.body : JSON.stringify(req.body || "");
        if (!body || body.length < 5) return sendErr(res, "Body JSON kosong");

        let decoded;
        try { decoded = JSON.parse(body); } catch (e) { return sendErr(res, "JSON tidak valid: " + e.message); }
        if (!Array.isArray(decoded)) return sendErr(res, "Format JSON tidak dikenal");
      ...
      case "upload_json_chunk": {
        ...
        if (!nama) return sendErr(res, '"nama" wajib diisi');
        if (!uploadId) return sendErr(res, '"upload_id" wajib diisi');
        if (chunkIndex < 1) return sendErr(res, '"chunk_index" harus >= 1');
        if (totalChunks < 1) return sendErr(res, '"total_chunks" harus >= 1');

        let chunkData = typeof req.body === "string" ? req.body : JSON.stringify(req.body || "");
        if (!chunkData) return sendErr(res, "Chunk data kosong");
        ...
        if (receivedCount < totalChunks) {
          return sendOk(res, {
            chunk_received: chunkIndex, total_chunks: totalChunks,
            chunks_so_far: receivedCount, complete: false,
        ...
        if (allChunks.rows.length !== totalChunks) return sendErr(res, "Gagal menggabungkan chunk");
        ...
        try { decoded = JSON.parse(fullJson); } catch (e) {
          ...
          return sendErr(res, "JSON tidak valid: " + e.message);
        }
        if (!Array.isArray(decoded)) {
          await db.execute({ sql: `DELETE FROM chunks WHERE upload_id = ?`, args: [uploadId] });
          return sendErr(res, "Format JSON tidak dikenal");
        }
      ...
      case "delete_route": {
        ...
        return sendOk(res, { deleted_id: id, nama: route.rows[0].nama });
      }

      default:
        return sendErr(res, "Action tidak dikenal. Tersedia: ping, upload_json_raw, upload_json_chunk, get_json, get_json_preview, list_routes, get_route_info, delete_route, search");
    }
  } catch (e) {
    return sendErr(res, "Server error: " + e.message, 500);
  }
};
